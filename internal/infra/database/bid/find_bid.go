...
import (
	...
	"github.com/diogocardoso/go_lab3/internal/entity/bid_entity"
...
func (bd *BidRepository) FindBidByAuctionId(ctx context.Context, auctionId string) ([]bid_entity.Bid, *internal_error.InternalError) {
	...
	if err != nil {
		logger.Error(fmt.Sprintf("Error trying to find bid by auction id %s", auctionId), err)
		return nil, internal_error.NewInternalServerError(fmt.Sprintf("Error trying to find bid by auction id %s", auctionId))
	}
